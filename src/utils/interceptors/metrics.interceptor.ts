...
export class MetricsInterceptor implements NestInterceptor {
  ...
  intercept(executionContext: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    this.counter.add(1, { [SemanticAttributes.HTTP_URL]: request.url, [SemanticAttributes.HTTP_METHOD]: request.method })
    ...
    return next.handle().pipe(
      tap(() => {
        ...
        this.histogram.record(executionTime, { [SemanticAttributes.HTTP_URL]: request.url, [SemanticAttributes.HTTP_METHOD]: request.method, [SemanticAttributes.HTTP_STATUS_CODE]: res.statusCode });
      })
    );
  }
}

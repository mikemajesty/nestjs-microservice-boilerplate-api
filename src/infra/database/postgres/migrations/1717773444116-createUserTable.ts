...
export class CreateUsersTable1717773444116 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const typeExists: { exists: boolean }[] = await queryRunner.query(
      `SELECT true as exists FROM pg_type WHERE typname = 'users_roles_enum'`
    );
    if (!typeExists.length) {
      await queryRunner.query(`CREATE TYPE users_roles_enum AS ENUM ('${Object.values(UserRoleEnum).join("','")}');`);
    }
    await queryRunner.query(
      `CREATE TABLE IF NOT EXISTS "users" ("id" uuid NOT NULL, "name" text NOT NULL, "email" text NOT NULL, "roles" "public"."users_roles_enum" array NOT NULL, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "deleted_at" TIMESTAMP, "password_id" uuid, CONSTRAINT "UQ_97672ac88f789774dd47f7c8be3" UNIQUE ("email"), CONSTRAINT "REL_4175c832d328c98ebafbc82aa9" UNIQUE ("password_id"), CONSTRAINT "PK_a3ffb1c0c8416b9fc6f907b7433" PRIMARY KEY ("id"))`
    );
